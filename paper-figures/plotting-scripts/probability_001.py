    ...
    def __init__(
        ...
        if isinstance(weights, str):
            if weights == 'sameheight':
                ...
                # weights[weights < 0] = 0
                self.weights = normalize(weights)  # type: ignore
...
class TernaryClassifier(ABC):
    ...
    def plot_certainty_region(
        ...
        red_verts = [(xs[0], 0), *zip(xs, red_zone), (xs[-1], 0)]
        red_poly = Polygon(red_verts, facecolor=(1, 0, 0, .3), edgecolor=c)
        ax.add_patch(red_poly)

        blue_verts = [(xs[0], 0), *zip(xs, blue_zone), (xs[-1], 0)]
        blue_poly = Polygon(blue_verts, facecolor=(0, 0, 1, .3), edgecolor=c)
...
def wing_BIWT(
    ...
    red_given_dist = [
        BernoulliDistribution(1 if i < n_red else 0)  # P[red|dist] = 1 if dist is on the tail
        for i in range(n)
    ]

    return BayesianInferenceWithThreshhold(
        dists, p_dist, red_given_dist, confidence, threshhold)
...
class BayesianInferenceWithThreshhold(TernaryClassifier):
    def __init__(
        ...
        red_given_dist: List[BernoulliDistribution],  # P[E=red|θ=θ₁]  # Discrete (red or not red)
        confidence: float,
        threshhold: float
    ) -> None:
        # self.n = n
        self.dists = dists
        self.red_given_dist = red_given_dist
        self.p_dist = p_dist
        self.threshhold = threshhold
        self.confidence = confidence
        # self.neither_p = (1 / confidence) - 1
    ...
    def p_red_p_x_given_red(self, x: float) -> float:
        return sum(
            dist.pdf(x) * self.p_dist.pdf(i) * self.red_given_dist[i].pdf(1)
    ...
    def p_blue_p_x_given_blue(self, x: float) -> float:
        return sum(
            dist.pdf(x) * self.p_dist.pdf(i) * self.red_given_dist[i].pdf(0)
            for i, dist in enumerate(self.dists)
        )
